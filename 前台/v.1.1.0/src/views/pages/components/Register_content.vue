<template>
  <form-wizard ref="checkoutWizard"
    ...
    <!-- tab 2 content -->
    <tab-content title="填写信息"
                 class="mb-5"
                 :before-change="validateStep2">
      <form data-vv-scope="step-2">
        <div class="vx-row justify-center items-center">
          <div class="vx-col md:w-1/2 w-full">
            <vs-input v-validate="'required|min:3|max:16'"
                      data-vv-validate-on="blur"
                      placeholder="用户名"
                      name="用户名"
                      icon-no-border
                      ...
                      v-model="username"
                      class="w-full" />
            <span class="text-danger text-sm">{{ errors.first('step-2.用户名') }}</span>
...
export default {
  ...
  computed: {
    ...
    validatsecond () {
      return !this.errors.any() && this.username != '' && this.password != '' && this.password2 != '' && this.verification_mail != '' === true;
    }
  },
  methods: {
    refimg_get () {
    ...
    validateStepsecond () {
      ...
      axios({
        ...
        data: {
          ...
          emailCode: this.verification_mail,
        }
      })
        .then((response) => {
          if (response.data["Status"] == "0") {
